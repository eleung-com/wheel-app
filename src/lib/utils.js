...
export const LS_FINNHUB_KEY  = 'wd_finnhub_key';
...
export function getFinnhubKey()  { return localStorage.getItem(LS_FINNHUB_KEY) || ''; }
...
export function normalizeDate(val) {
  ...
  // Google Sheets date serial: integer in the range 40000–80000 (~2009–2118)
  const n = Number(s);
  if (!isNaN(n) && Number.isInteger(n) && n > 40000 && n < 80000) {
    // GS epoch is 1899-12-30 (not 1900-01-01 — off-by-2 Lotus compat)
...
export function parsePositions(raw) {
  return raw.filter(p => p.ticker).map(p => ({
