...
async function fetchHistoryYahoo(ticker) {
  // Local: Vite proxy at /yf sets a browser User-Agent so Yahoo doesn't reject the request.
  // Production (GitHub Pages): Yahoo sends no CORS headers, so a direct browser fetch is
  // blocked — route through the Cloudflare Worker's /yf proxy (see worker/README.md).
  const isLocal = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
  const yahooBase = isLocal ? '/yf' : 'https://wheel-tradier-proxy.esthercandy.workers.dev/yf';
